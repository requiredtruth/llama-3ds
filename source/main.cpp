...
static void home(int sel,const Runtime& rt,bool net){
    consoleClear(); std::printf("llama-3ds v0.2.0\nNew 3DS / New 2DS XL local LLM\n\n");
...
static void models(Runtime& rt,bool net){
    int sel=0; const int count=sizeof(catalog)/sizeof(catalog[0]);
    while(aptMainLoop()){
        ...
        hidScanInput(); u32 d=hidKeysDown(); if(d&KEY_B) return; if(d&KEY_DUP) sel=(sel+count-1)%count; if(d&KEY_DDOWN) sel=(sel+1)%count;
        const auto& m=catalog[sel]; std::string path=std::string(kModels)+"/"+m.filename;
        if(d&KEY_X){ rt.unload(); std::remove(path.c_str()); std::remove((path+".part").c_str()); }
        if(d&KEY_A){
            if(!exists(path)){
                consoleClear(); std::printf("Download\n%s\n\n",m.title);
                if(!net){ std::printf("HTTP service unavailable.\n"); wait_back(); continue; }
                auto r=download_verified(m.url,path,m.sha256); std::printf("\n%s\n",r.message.c_str()); if(!r.ok){ wait_back(); continue; }
            } else if(!verify_file(m,path)) continue;
...
static void chat(Runtime& rt){
    ...
    while(aptMainLoop()){
        ...
        frame(); hidScanInput(); u32 d=hidKeysDown(); if(d&KEY_B) return; if(d&KEY_X) log.clear(); if(!(d&KEY_A)||!rt.loaded()) continue;
        ...
        if(swkbdInputText(&kb,input,sizeof(input))!=SWKBD_BUTTON_RIGHT || !input[0]) continue;
        log.push_back(std::string("You: ")+input); consoleClear(); std::printf("Generating locally...\n"); frame();
        std::string answer,err; if(rt.generate(input,answer,err)) log.push_back("LLM: "+answer); else log.push_back("ERROR: "+err);
    }
}
}
...
int main(int,char**){
    ...
    while(aptMainLoop()){
        llama3ds::home(sel,runtime,net); llama3ds::frame(); hidScanInput(); u32 d=hidKeysDown();
        if(d&KEY_START) break; if(d&KEY_DUP) sel=(sel+3)%4; if(d&KEY_DDOWN) sel=(sel+1)%4;
        if(d&KEY_A){ if(sel==0) llama3ds::models(runtime,net); else if(sel==1) llama3ds::chat(runtime); else if(sel==2) llama3ds::system_screen(); else llama3ds::about(); }
    }
